...
//
        ...
        // 4 + pow(5+6, pow(3 / 2, 6 + 4))
        var application = new MathApplication();
        var context = new MathContext(List.of("sin", "cos"));
        application.runLab2("(2 * x^2-5 * x+7)", context);
    }

    public void runLab2(String input, MathContext context) {
        if (input.length() > 500) {
        ...
        var transform = transformer.transform(syntaxTokens);
        System.out.println("---Postfix---");
        transform.forEach(System.out::println);
    }

    public void runLab1(String input, MathContext context) {
        if (input.length() > 500) {
            System.out.println("Input string is too long");
            return;
        }
